...
def serialize_mat(m):
    ...
    return {
        ...
        "created_at": m["created_at"].strftime("%d %b %Y") if hasattr(m.get("created_at"), "strftime") else str(m.get("created_at","")),
...
def get_materials():
    ...
    if role == "admin":
        ...
        # Marketing sees their own uploads + all approved
        q = {"$or": [
            {"uploaded_by_id": uid},
            {"status": "approved"}
        ]}
    else:
        # All other staff: only approved materials
        q = {"status": "approved"}

    # Apply filters on top
    if status_filter and role == "admin":
        q = {"status": status_filter}
    if solution_filter:
        if "$or" in q:
            # wrap with $and
            q = {"$and": [q, {"solution": solution_filter}]}
        else:
...
@jwt_required()
def get_library():
    # Library = approved materials only, visible to ALL logged-in users
    solution_filter = request.args.get("solution","")
    q = {"status": "approved"}
    if solution_filter:
        q["solution"] = solution_filter
...
def create_material():
    ...
    # Marketing dept OR admin OR marketing role can upload
