...
export default function AuthPage() {
  ...
  return (
    <div style={{display:'flex', minHeight:'100vh', background:'#F4F4F5'}}>
      {/* Left panel */}
      <div style={{width:'45%', background:'var(--navy)', display:'flex', flexDirection:'column', justifyContent:'space-between', padding:'48px', position:'relative', overflow:'hidden'}}>
        <div style={{position:'absolute', top:0, left:0, right:0, height:3, background:'linear-gradient(90deg,#14F1B1,#114EFF,#091526)'}}/>

        {/* Logo */}
        <div>
          <img src="/static/logo.svg" alt="Bizaxl" style={{height:36, filter:'brightness(0) invert(1)'}} />
          <div style={{marginTop:12, height:2, width:48, background:'#14F1B1', borderRadius:2}}/>
        </div>

        {/* Quote */}
        <div>
          <div style={{fontSize:11, fontWeight:700, letterSpacing:'1.5px', textTransform:'uppercase', color:'#14F1B1', marginBottom:16}}>
            Today's Thought
          </div>
          <p style={{fontSize:20, fontWeight:600, color:'white', lineHeight:1.6, fontStyle:'italic', marginBottom:16}}>
            "{quote.text}"
          </p>
          <p style={{fontSize:12, color:'rgba(255,255,255,0.4)', textTransform:'uppercase', letterSpacing:'0.5px'}}>
            — {quote.attr}
          </p>
        </div>
        ...
        <div>
          <p style={{fontSize:12, color:'rgba(255,255,255,0.3)'}}>
            Bizaxl Workspace · Internal Platform
          </p>
          <p style={{fontSize:11, color:'rgba(255,255,255,0.2)', marginTop:4}}>
            Building confidence, dignity and growth.
          </p>
        </div>
      </div>
      ...
      <div style={{flex:1, display:'flex', alignItems:'center', justifyContent:'center', padding:'48px'}}>
        <div style={{width:'100%', maxWidth:420}}>
          <h1 style={{fontSize:28, fontWeight:700, color:'var(--navy)', marginBottom:6}}>
          ...
          {/* Mode toggle */}
          <div style={{display:'flex', gap:0, marginBottom:28, background:'var(--gray-100)', borderRadius:'var(--radius)', padding:4}}>
