...
export default function IdeasPage() {
  ...
  const resetForm = () => { setForm({title:'',description:'',tags:''}); setEditIdea(null); setShowForm(false); };

  const submit = async (e) => {
    e.preventDefault();
    const tags = form.tags.split(',').map(t=>t.trim()).filter(Boolean);
    if (editIdea) {
      await axios.put(`${API}/ideas/${editIdea.id}`, {...form, tags});
    } else {
  ...
  const startEdit = (idea) => {
    setEditIdea(idea);
    setForm({ title:idea.title, description:idea.description, tags:(idea.tags||[]).join(', ') });
    setShowForm(true);
  ...
  const deleteIdea = async (id) => {
    ...
    await axios.delete(`${API}/ideas/${id}`); fetch();
  };
  ...
  const setStatus = async (id, status) => {
    await axios.put(`${API}/ideas/${id}/status`, { status }); fetch();
  };
    ...
    <div className="page-container">
      <div className="flex-between page-header">
        ...
        <div style={{display:'flex',gap:8}}>
          ...
          <button className="btn btn-primary" onClick={()=>{ resetForm(); setShowForm(!showForm); }}>
            {showForm && !editIdea ? 'Cancel' : '+ Share Idea'}
          </button>
        </div>
      </div>
      ...
      {showForm && (
        <div className="card" style={{marginBottom:24,borderTop:'3px solid var(--mint)'}}>
          <h3 style={{fontWeight:700,marginBottom:20,fontSize:16}}>{editIdea?'Edit Idea':'Share Your Idea'}</h3>
          <form onSubmit={submit}>
            <div className="form-group">
              <label className="label">Title *</label>
              <input className="input" placeholder="A short, clear title" value={form.title} onChange={e=>setForm(f=>({...f,title:e.target.value}))} required/>
            </div>
            <div className="form-group">
              <label className="label">Description *</label>
              <textarea className="input textarea" rows={4} placeholder="Describe your idea in detail..." value={form.description} onChange={e=>setForm(f=>({...f,description:e.target.value}))} required/>
            </div>
            <div className="form-group">
              <label className="label">Tags (comma separated)</label>
              <input className="input" placeholder="e.g. automation, process, culture" value={form.tags} onChange={e=>setForm(f=>({...f,tags:e.target.value}))}/>
            </div>
            <div style={{display:'flex',gap:8}}>
              <button type="submit" className="btn btn-primary">{editIdea?'Save Changes':'Post Idea'}</button>
      ...
      <div style={{display:'flex',flexDirection:'column',gap:12}}>
        {ideas.length===0 && (
          <div className="card"><div className="empty-state">
            ...
            <h3>No ideas yet</h3><p>Be the first to share one</p>
          </div></div>
        )}
        {ideas.map(idea=>{
          const isOwner = idea.author_id===user.id || idea.author_name===user.name;
                ...
                <div style={{display:'flex',gap:8,alignItems:'center',flexWrap:'wrap'}}>
                  ...
                  {canManage&&(
                    <div style={{display:'flex',gap:4}}>
                      {isOwner&&<button onClick={()=>startEdit(idea)} className="btn btn-outline btn-sm" style={{fontSize:12,padding:'4px 10px'}}>Edit</button>}
                      <button onClick={()=>deleteIdea(idea.id)} className="btn btn-danger btn-sm" style={{fontSize:12,padding:'4px 10px'}}>Delete</button>
                    </div>
                  )}
                </div>
              </div>
                ...
                <div style={{marginTop:14,paddingTop:14,borderTop:'1px solid var(--border)'}}>
                  {idea.comments?.map((c,i)=>(
                    <div key={i} style={{padding:'8px 0',borderBottom:'1px solid var(--border)',fontSize:13}}>
                      <span style={{fontWeight:600,color:'var(--navy)'}}>{c.author}</span>{' '}
                      <span style={{color:'var(--gray-400)'}}>{c.text}</span>
                    </div>
                  ))}
                  <div style={{display:'flex',gap:8,marginTop:10}}>
                    <input className="input" placeholder="Write a comment..." style={{flex:1}} value={commentText[idea.id]||''} onChange={e=>setCommentText(c=>({...c,[idea.id]:e.target.value}))}
                      onKeyDown={e=>e.key==='Enter'&&comment(idea.id)}/>
