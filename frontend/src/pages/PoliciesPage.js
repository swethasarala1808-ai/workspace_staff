...
export default function PoliciesPage() {
  ...
  const create = async (e) => {
    ...
    setShowForm(false); fetch(); setMsg('Policy published'); setTimeout(()=>setMsg(''),3000);
  };
    ...
    <div className="page-container">
      <div className="flex-between page-header">
        <div>
          ...
          <p className="page-subtitle">{unread > 0 ? `${unread} unread policies` : 'All policies acknowledged'}</p>
        </div>
        {user.role==='admin' && (
      ...
      {showForm && user.role==='admin' && (
        <div className="card" style={{marginBottom:24, borderTop:'3px solid var(--mint)'}}>
          <h3 style={{fontWeight:700, marginBottom:20, fontSize:16}}>Create Policy</h3>
          <form onSubmit={create}>
            ...
            <div className="form-group"><label className="label">Summary</label><input className="input" value={form.summary} onChange={e=>setForm(f=>({...f,summary:e.target.value}))} required/></div>
            <div className="form-group"><label className="label">Full Content</label><textarea className="input textarea" rows={6} value={form.content} onChange={e=>setForm(f=>({...f,content:e.target.value}))} required/></div>
            <div style={{display:'flex', gap:8}}>
      ...
      {selected ? (
        <div className="card">
          ...
          <div style={{display:'flex', gap:16, marginBottom:20, alignItems:'flex-start'}}>
            <div style={{width:48, height:48, background:'var(--gray-100)', borderRadius:'var(--radius)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:24, flexShrink:0}}>{selected.emoji||'📋'}</div>
            <div>
          ...
          <div style={{background:'var(--gray-100)', borderRadius:'var(--radius-lg)', padding:20, fontSize:14, lineHeight:1.9, whiteSpace:'pre-wrap', marginBottom:20, color:'var(--navy)'}}>{selected.content}</div>
          {!selected.read
        ...
        <div style={{display:'flex', flexDirection:'column', gap:8}}>
          {policies.length===0 && (
            <div className="card"><div className="empty-state"><div className="empty-state-icon">📋</div><h3>No policies yet</h3>{user.role==='admin'&&<p>Click "Seed Defaults" to add standard policies</p>}</div></div>
          )}
          {policies.map(p=>(
            <div key={p.id} className="card" onClick={()=>setSelected(p)}
              ...
              <div style={{width:36, height:36, background:'var(--gray-100)', borderRadius:'var(--radius)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:18, flexShrink:0}}>{p.emoji||'📋'}</div>
